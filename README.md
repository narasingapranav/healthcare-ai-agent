...
10. [AI Agent & Database RAG Architecture](#ai-agent--database-rag-architecture)
...
### AI & Agent Integration (Upgraded)
✅ **Tool-Calling Agent**: Powered by LangChain compiled state graphs (`create_agent`). The agent dynamically routes commands to database write/read tools and external API lookups.
✅ **Dynamic Database RAG**: Searches and indexes all 8 MongoDB collections (medications, goals, metrics, preferences, insurance, history, nutrition, Indian catalog) in real-time using `InMemoryVectorStore` or a zero-cost local TF-IDF similarity matcher.
✅ **Context-Aware Prompts**: Injects dynamic patient summary overviews directly into the system message on every conversation turn.
✅ **Medication Interaction Warnings**: Real-time alerts and checks for known drug combinations.
...
| **LLM Integration** | LangChain Agent Graphs (with Groq/OpenAI/Google APIs) |
...
## 🤖 AI Agent & Database RAG Architecture

The Healthcare AI Agent has been upgraded to a **compiled tool-calling state graph agent** integrated with a **MongoDB database-wide RAG system**. This architecture enables the agent to search through past health history, update patient records, and lookup medical databases in response to user text queries.

```mermaid
graph TD
    User([User Chat Input]) --> AgentGraph[Compiled State Graph Agent]
    AgentGraph --> PromptGenerator[Dynamic System Prompt Generator]
    PromptGenerator --> DBFetch[MongoDB Fetch Stats]
    DBFetch --> PatientSummary[Dynamic Patient Status Overview]
    PatientSummary --> ModelCall[LLM Decision Node]
    
    ModelCall -->|Choose Action| ToolNode[Tool Execution Node]
    ToolNode -->|CRUD Query/Write| MongoDB[(MongoDB Collections)]
    ToolNode -->|External Fetch| ExternalAPIs[Practo / 1mg / MedlinePlus]
    ToolNode -->|Database Search| RAGEngine[Dynamic TF-IDF / Vector RAG]
    RAGEngine --> MongoDB
    
    ToolNode -->|Return Result| ModelCall
    ModelCall -->|Generate Text| User
```

### 1. Dynamic Database RAG System (`src/rag.py`)

The dynamic RAG engine indexes and retrieves details across all MongoDB collections. Rather than relying on a static index, the RAG engine performs **on-the-fly indexing** to guarantee that the agent always has access to the most up-to-date patient records:
- **Serialization**: Converts structured MongoDB documents (medications, goals, logs, medical history, regional preferences, insurance profiles) into detailed textual paragraphs.
- **Search System**:
  - **Embedding-Based Vector Search**: Utilizes LangChain's `InMemoryVectorStore` combined with `OpenAIEmbeddings` or `GoogleGenAIEmbeddings` if API keys are configured.
  - **Local Cosine-Similarity Search Fallback**: Features a zero-cost local TF-IDF cosine similarity search engine that executes in pure Python. This ensures RAG queries are processed instantly without requiring embedding API calls or external server configurations.

### 2. Custom LangChain Tools (`src/agent_tools.py`)

The agent is equipped with 20 custom LangChain `@tool` functions that connect the model to MongoDB, lookup databases, and safety modules:
- **Database Query (Read) Tools**:
  - `get_user_medications()`: Reads scheduled medications.
  - `get_health_metrics()`: Reads vital signs and logged metrics.
  - `get_health_goals()`: Reads active target health benchmarks.
  - `get_nutrition_logs()`: Reads recent food and macro details.
  - `get_medical_history()`: Reads patient diagnoses and treatments.
  - `get_insurance_profile()`: Reads insurer details and policy bounds.
  - `get_regional_preferences()`: Reads language, location, and doctor budget limits.
- **Database Action (Write/Create) Tools**:
  - `schedule_new_medication(name, dosage, schedule_time, notes)`
  - `log_new_health_metric(metric_name, metric_value, unit, recorded_at)`
  - `add_new_health_goal(metric_name, target_value, unit)`
  - `log_nutrition(meal_date, meal_type, region, food_item, ...)`
  - `add_medical_history(patient_name, condition_name, diagnosis_date, ...)`
  - `add_insurance_profile(patient_name, insurer, policy_number, ...)`
  - `add_regional_preferences(patient_name, state, city, ...)`
- **Safety, RAG, & External Lookup Tools**:
  - `search_health_records(query)`: Dynamically searches the MongoDB RAG index.
  - `check_medication_interactions(medication_names)`: Checks drug interaction warnings.
  - `search_indian_medications(query)`: Queries 1mg medicine catalog.
  - `search_local_doctors(city, specialty)`: Queries Practo provider directory.
  - `lookup_ayurvedic_remedies(remedy_name)`: Looks up traditional herbs and remedies.
  - `lookup_general_medical_info(topic)`: Summarizes conditions with WHO/CDC citations.

### 3. State Graph Agent & Injections (`src/chatbot.py`)

- **State Graph Agent**: Utilizes LangChain's compiled `create_agent` graph that loops between model decision nodes and tool execution nodes until all tool calls are completed.
- **Dynamic Context Injection**: On every message turn, the chatbot calls `get_patient_summary_string()` to generate a concise summary of the patient's active status (e.g. active medications, goals, latest vital signs). This summary is dynamically injected directly into the system message of the LLM context, keeping the agent fully context-aware without requiring additional tool calls.
- **Session-Based Chat History**: Dynamic message history parser formats Streamlit session messages into LangChain `HumanMessage` and `AIMessage` classes for turn-based chat memory.
